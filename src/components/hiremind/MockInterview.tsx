...
export function MockInterview({
  ...
  // DYNAMIC SCRIPT
  const script = [
    ...(analysisData?.questions?.hr || []).map(
      (q: string) => ({
        q,
        topic: "HR Intelligence",
      })
    ),

    ...(analysisData?.questions?.technical || []).map(
      (q: string) => ({
        q,
    ...
    ...(analysisData?.questions?.behavioral || []).map(
      (q: string) => ({
        q,
  ...
  if (!analysisData || script.length === 0) {
    return null;
  }
  ...
  const submit = () => {
    ...
    setTimeout(() => {
      ...
      const star =
        /\b(situation|task|action|result|because|when|then)\b/i.test(
          answer
        );
      ...
      setFeedback({
        ...
        depth: Math.min(
          92,
          50 + Math.min(40, len * 0.8)
        ),

        confidence: Math.min(
          94,
          55 + Math.min(35, len * 0.6)
        ),

        note: star
          ? "Strong STAR structure detected. Quantify the final impact more clearly."
          : "Good direction. Improve structure using Situation → Action → Result framing.",
      });
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onClose}
        className="absolute top-6 right-6 text-muted-foreground hover:text-foreground text-sm z-10"
      >
        close
      </button>
      ...
      <div className="relative max-w-4xl w-full">
        ...
        <div className="relative">
          <div className="text-xs uppercase tracking-[0.3em] text-primary/80 mb-3">
            {current.topic}
          </div>
